...
class SkipGramModel(nn.Module):
    ...
    def init_emb(self):
        ...
        The u_embedding is a uniform distribution in [-0.5/emb_dimension, 0.5/emb_dimension], and the elements of v_embedding are zeroes.
...
class CBOW(nn.Module):
    ...
    def forward(self, pos_u, pos_v, neg_u, neg_v):
        ...
        losses = []
        emb_u = self.u_embeddings(Variable(torch.LongTensor(pos_u_m)))
        emb_v = self.v_embeddings(Variable(torch.LongTensor(pos_v)))
        score = torch.mul(emb_u, emb_v)
        score = torch.sum(score, dim=1)
        score = F.logsigmoid(score)
        losses.append(sum(score))
        neg_emb_u = self.u_embeddings(Variable(torch.LongTensor(neg_u_m)))
        neg_emb_v = self.v_embeddings(Variable(torch.LongTensor(neg_v)))
        neg_score = torch.mul(neg_emb_u, neg_emb_v)
        neg_score = torch.sum(neg_score, dim=1)
        neg_score = F.logsigmoid(-1 * neg_score)
